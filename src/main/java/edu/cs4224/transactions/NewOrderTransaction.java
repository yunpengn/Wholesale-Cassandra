...
public class NewOrderTransaction extends BaseTransaction {
  ...
  private void createNewOrder(List<Integer> itemIds, List<Integer> supplierWareHouse, List<Integer> quantity) {
    ...
    for (int i = 0; i < numDataLines; i++) {
      ...
      double quantity_left = ScalingParameters.fromDB(stock_info.getLong("S_QUANTITY"),
          ScalingParameters.SCALE_S_QUANTITY);
      ...
      String update_stock_query = String.format(CqlQueryList.UPDATE_STOCK,
          ScalingParameters.toDB(adjusted_quantity - quantity_left, ScalingParameters.SCALE_S_QUANTITY),
          ScalingParameters.toDB(quantity.get(i), ScalingParameters.SCALE_S_YTD),
    ...
    String check_customer_discount_query = String.format(CqlQueryList.CHECK_CUSTOMER_INFO, warehouseID, districtID,
        customerID);
    ...
    for (int i = 0; i < numDataLines; i++) {
      System.out.println(String.format(
          "\t ITEM_NUMBER: %s, I_NAME: %s, SUPPLIER_WAREHOUSE: %d, QUANTITY: %d, OL_AMOUNT: %.2f, S_QUANTITY: %f",
          itemIds.get(i), itemsName.get(i), supplierWareHouse.get(i), quantity.get(i), itemsAmount.get(i),
